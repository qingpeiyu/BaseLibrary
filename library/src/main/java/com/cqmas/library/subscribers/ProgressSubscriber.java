package com.cqmas.library.subscribers;
 ...
 * 调用者自己对请求数据进行处理
 */
public class ProgressSubscriber<T> extends ResourceObserver<T> {
...
//                    /*获取缓存数据*/
//                    CookieResulte cookieResulte = DbCookieUtil.getInstance().queryCookieBy(s);
...
//                        }
//                    } else {
//                        DbCookieUtil.getInstance().deleteCookie(cookieResulte);
//                        throw new HttpTimeException("网络错误");
//                    }
//                }
//            });
...
//             /*获取缓存数据*/
//            CookieResulte cookieResulte = DbCookieUtil.getInstance().queryCookieBy(api.getUrl());
//            if (cookieResulte != null) {
//                long time = (System.currentTimeMillis() - cookieResulte.getTime()) / 1000;
